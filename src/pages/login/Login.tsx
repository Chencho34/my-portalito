    ...
    <main className='grid grid-flow-row grid-cols-12 h-screen'>
      <article className='hidden md:grid md:col-span-5 bg-hero-login bg-cover bg-no-repeat object-cover'>
        <section className='grid place-content-end text-white h-full bg-gradient-to-t from-[rgb(33,33,33,.9)] to-[rgb(33,33,33,.4)] bg-transparent p-8 overflow-hidden'>
          <p className='flex flex-col text-wrap text-5xl tracking-[2px] leading-tight mb-6 font-semibold overflow-hidden text-left'>
            <span className='font-normal'>Hello,</span> 
            <span className=''>welcome! To </span>
            <span className='text-[#9811c5] font-normal'>MyPortal ITO</span>
          </p>
          <p className='text-sm leading-snug font-normal text-gray-300 tracking-wider mb-6 line-clamp-6 text-pretty'>
          ...
          <button type='button' className='w-[140px] py-2 rounded border-2 border-neutral-50  text-sm font-normal  leading-normal text-neutral-50 transition duration-150 ease-in-out hover:border-neutral-100 hover:bg-neutral-100 hover:bg-opacity-10 hover:text-neutral-100'>
            Get started
          </button>
        </section>
      </article>
        ...
        <nav className='flex items-center w-full h-20 absolute top-0 right-0 left-0'>
          <section className='flex items-center justify-between h-full w-[80%] mx-auto'>
            ...
            <ul className='hidden md:flex gap-6 text-sm font-medium'>
              <li><a href='#'><FaSquareFacebook size={21} color='#3b5998' /></a></li>
              <li><a href='#'><FaInstagram size={21} color='#E1306C' /></a></li>
              <li><a href='#'><FaXTwitter size={21} color='#212121' /></a></li>
            </ul>
          </section>
        </nav>
        <article className='flex flex-col items-center justify-center'>
          <form className='flex flex-col gap-6 bg-white rounded-lg shadow-lg p-8 w-full max-w-xs' onSubmit={handleSubmit}>
            ...
            <ul className='md:hidden flex justify-around bg-slate-5  rounded gap-6 text-sm font-medium'>
              <li><a href='#'><FaSquareFacebook size={20} color='#3b5998' /></a></li>
              <li><a href='#'><FaInstagram size={20} color='#E1306C' /></a></li>
